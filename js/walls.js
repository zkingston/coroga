var roofCfg = { attr: { color : 0x404040,
                        shading : THREE.FlatShading,
                        shininess : 30,
                        refractionRatio : 0.5 } };

var wallCfg = { offset : brickCfg.offset * 7 + brickCfg.offset / 4,
                attr: { color : 0xfefcb4,
                        shading : THREE.FlatShading,
                        shininess : 5,
                        refractionRatio : 0.1 } };
...
function createWalls( width, height, wallHeight ) {
    ...
    wallN.addToObject( obj, 0, height / 2 + wallOff / 2, wallHeight / 2 - 6 * brickOff / 4 );
    wallS.addToObject( obj, 0, -( height / 2 + wallOff / 2 ), wallHeight / 2 - 6 * brickOff / 4 );
    ...
    wallE.addToObject( obj, width / 2 + wallOff / 2, 0, wallHeight / 2 - 6 * brickOff / 4 );

    wallW.rotation.z += Math.PI / 2;
    wallW.addToObject( obj, -( width / 2 + wallOff / 2 ), 0, wallHeight / 2 - 6 * brickOff / 4 );
    ...
    obj.addFeatureGeometry( 'gravel', new THREE.PlaneGeometry( width + 4 * brickOff,
                                                               height + 4 * brickOff,
                                                               width * 3, height * 3 ) );
    obj.addFeatureMaterialP( 'gravel', { color : 0x453834,
                                         shading : THREE.FlatShading,
                                         shininess : 60,
                                         refractionRatio : 0.8 } );
    ...
    obj.addFeatureMaterialL( 'floor', { color : 0xede2cb } );
    obj.getFeature( 'floor' ).geometry.translate( 0, 0, -brickCfg.offset );
...
function WallSegmentGeometry( width, height, depth ) {
    var wall = new THREE.BoxGeometry( width, 3 * height / 4, 3 * depth / 4, 3, 3, 3 );
